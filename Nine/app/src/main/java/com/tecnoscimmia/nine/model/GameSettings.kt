 ...
 * for example the gameMode is not saved)
 */
...
class GameSettings private constructor(private val preferencesFile: SharedPreferences)
{
	...
	enum class SymbolsSetSetting		{ NUMBERS_SYMBOLS_SET, LETTERS_SYMBOLS_SET, EMOTICONS_SYMBOLS_SET, PUNCTUATION_SYMBOLS_SET, MIXED_SYMBOL_SET }
	enum class DebugModeSetting			{ DEBUG_MODE, NO_DEBUG_MODE }
	...
	private var debugMode:				DebugModeSetting = DebugModeSetting.NO_DEBUG_MODE						// Indicates the type of debug mode that the app is running into
	...
	fun getDebugMode() :				DebugModeSetting		{ return debugMode }


	fun isDebugModeActive() : Boolean
	{
		return debugMode != DebugModeSetting.NO_DEBUG_MODE
	}
	...
	fun setDebugMode(newMode: DebugModeSetting)
	{
		debugMode = newMode
		preferencesFile.edit().putString(debugModePreferencesKey, newMode.name).apply()
	}
	...
	private fun loadFromFile()
	{
		...
		if(preferencesFile.contains(themePreferencesKey))
		{
			...
			debugMode = DebugModeSetting.valueOf(
				preferencesFile.getString(debugModePreferencesKey, DebugModeSetting.NO_DEBUG_MODE.name) as String)
			...
			debugMode		= DebugModeSetting.NO_DEBUG_MODE
			...
			edit.putString(debugModePreferencesKey, debugMode.name)
			edit.apply()
		}
	}
